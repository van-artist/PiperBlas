...
static void *pi_spmv_worker(void *arg_)
{
    ...
    const int *__restrict col_idx = arg->col_idx;
    const int *__restrict row_ptr = arg->row_ptr;
    const int row_begin = arg->row_begin;
    const int row_end = arg->row_end;
...
piState piSpMV(const pi_csr *__restrict A, double *__restrict x, double *__restrict y)
{
    ...
    int *cuts = (int *)malloc((size_t)(thread_num + 1) * sizeof(int));
    if (!cuts)
        return piErrAlloc;
    cuts[0] = 0;

    // 启发式思想，算出每个工作线程理想的处理的非0元素个数
    const int target = nnz / thread_num + 1;
    int acc = 0;
    int cur = 1;
    for (int r = 0; r < n_rows && cur < thread_num; ++r)
    {
